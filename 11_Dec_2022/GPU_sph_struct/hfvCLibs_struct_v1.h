...
//================ ngbDB_v3 =================
//========================================
__global__ void ngbDB_v3(Body *d_b, int *ngb, int MAX_ngb, int N)
{
  ...
  if ((i < N) && (d_b[i].Type == 0))
  {
    ...
    while (k < 65)
    {
      ...
      h_new = (2.0f + (counter + 1.0f) * coeff) * d_b[i].h;
      ...
      while (j < N && k < MAX_ngb)
      {
        if (d_b[j].Type == 0)
        {
          dx = d_b[j].pos.x - d_b[i].pos.x;
          dy = d_b[j].pos.y - d_b[i].pos.y;
          dz = d_b[j].pos.z - d_b[i].pos.z;
          rr = sqrt(dx * dx + dy * dy + dz * dz);
...
//========== Smoothing Length_ngb ============ Updated: 28 Jan 2023. h_new adopted from eq.31 in Gadget2 Paper
//========================================
__global__ void smoothing_h_ngb_v2(Body *d_b, int N, int Ndown, int Nup, float coeff,
                                   float Nngb_f, int *ngb, int MAX_ngb, int *ngbDebug,
                                   float dt)
{
  ...
  if ((i < N) && (d_b[i].Type == 0))
  {
    ...
    float h_new = 2.0f * (0.5f * d_b[i].h * (1.0f + pow((Nngb_f / d_b[i].Nngb_previous), 1.0f / 3.0f)) +
                          1.0f / 3.0f * d_b[i].h * d_b[i].divV * dt);
    int N_iter = 0;
    int k = 0;
    int j = 0;
    ...
    while ((k < Ndown) || (k > Nup))
    {
      ...
      if (k < Ndown)
      {
        h_new = h_new + coeff * 2.0f * d_b[i].h;
      }
