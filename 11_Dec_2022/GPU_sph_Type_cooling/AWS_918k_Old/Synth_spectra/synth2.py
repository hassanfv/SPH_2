...
from abundance_finder2 import getAbundance
...
unit_u = 1.5272E+13 
Unit_time_in_s = 7.897E+14 #seconds
...
#===== getDensityx
@njit
def getEnergy(r, pos, m, h, u, rho):  # r is the position of all particles. pos is the positions for which we want to know rho !
...
U = getEnergy(r, grid_vec, mass, h, u, rho) * unit_u
...
for i in range(len(nH_cgs)):

  Abund = getAbundance(U[i], nH_cgs[i], Z_p, dt_sec)

  nHI = Abund[1] * nH_cgs[i] * 0.01
  
  Ncol = dl_cgs * nHI
  
  if Ncol > 0.0:
    res.append(np.log10(Ncol))
  
  s += Ncol
  
  print(np.log10(Ncol+1e-1), Abund[1], nH_cgs[i])
