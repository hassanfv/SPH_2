...
filename = './Out918k/G-0.020000.bin'
...
print('sort(dt) = ', np.sort(dt))



#====== radial velocity plot =====

unit_velocity_cgs = 3907948.44 # cm/s

vr = np.sqrt(vx * vx + vy * vy + vz * vz)
r = np.sqrt(x * x + y * y + z * z)

print()
print('sort(r) = ', np.sort(r))
print()
nr = np.where((r < 0.35) & (r > 0.30))[0]
print('nr = ', nr)
print()
...
nT = np.where(Temp < 2450)[0]
print()
print('nT', nT)
print()
print('Temp[nT] = ', Temp[nT])
print()
rTT = np.sqrt(x[nT] * x[nT] + y[nT] * y[nT] + z[nT] * z[nT])
print('rr[nT] = ', rTT)
print()
print('nH[nT] = ', nH_cgs[nT])
print()

rr = np.sqrt(x * x + y * y + z * z)

#plt.scatter(rr, nH_cgs, s = 0.01)
#plt.title('r vs nH')
#plt.show()
...
#scatter = plt.scatter(x[nk], y[nk], c= 'k' , s=20)

#scatter = plt.scatter(x[nT], y[nT], c= 'lime' , s=20)
