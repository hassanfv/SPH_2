...
//*************************************************************************
void saveArraysToBinary(const std::string &filename, float *x, float *y, float *z,
                        float *rho, float *u, float *uBAd, float *uAC, float *mass, float *dudt, float *dudt_pre,
                        int *Typ, int N)
{
  ...
  file.write(reinterpret_cast<const char *>(uAC), N * sizeof(float));
...
//=====================================================================================
__global__ void hcooling(int *Typ, float *u, float *U, float *rho, float Zmetal, float dt, // Zmetal is the gass metallicity assumed.
                         float *nHGrid, float *ZGrid, float *uEvol, float *tArr_sec,
                         float *muA, float *logT, float unit_density_in_cgs, float unit_time_in_sec, 
                         float unit_u_in_cgs, float GAMMA_MINUS1, int N_nH, int N_Z, int N_T, int N_Time, int N)
{
  ...
  if ((i < N) && (Typ[i] == 0))
  {
    ...
    float mH = 1.6726e-24;
    float XH = 0.70;
    //float muT = 1.0; // For now we assume it to be 1.0 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    float rho_cgs = rho[i] * unit_density_in_cgs;
    //float nGas = rho_cgs / (muT * mH);
    //float nH = XH * nGas;
    ...
    for (int t = 0; t < N_T; t++)
    {
      int index = t * stride_T + ndx_nH * stride_nH + ndx_Z * stride_Z + 0;
      U[t] = uEvol[index];
    }
    ...
    int ndx = (ndx_u * stride_T) + (ndx_nH * stride_nH) + (ndx_Z * stride_Z) + ndx_t;
    
    if (ndx > 0)
    {
      int ndx_minus1 = ((ndx_u-1) * stride_T) + (ndx_nH * stride_nH) + (ndx_Z * stride_Z) + ndx_t;
