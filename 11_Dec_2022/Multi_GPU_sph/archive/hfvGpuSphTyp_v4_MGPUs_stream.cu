...
int main()
{
  ...
  //-------------- Smoothing Length ---------------
  //-----------------------------------------------
  for (int i = 0; i < nGPUs; i++) // Run kernel on each GPU.
  {

    cudaSetDevice(i);
    smoothing_h_mgpu<<<gridSize, blockSize, 0, streams[i]>>>(beg[i], end[i], d_Typ, d_x, d_y, d_z, d_h,
                                                             Ndown, Nup, coeff,
                                                             Nngb_f, d_Nngb_previous, d_divV, dt, N);
  }
  ...
  for (int i = 0; i < nGPUs; i++) // Run kernel on each GPU.
  {
    ...
    cudaSetDevice(i);
    getDensity_mgpu<<<gridSize, blockSize, 0, streams[i]>>>(beg[i], end[i], d_Typ, d_x, d_y, d_z, d_mass,
                                                            d_rho, d_h, N);
  }
  // Ensure all GPUs have finished.
  for (int i = 0; i < nGPUs; i++)
  ...
  for (int i = 0; i < nGPUs; i++) // Run kernel on each GPU.
  {
    ...
    cudaSetDevice(i);
    acc_g_mgpu<<<gridSize, blockSize, 0, streams[i]>>>(beg[i], end[i], d_Typ, d_x, d_y, d_z, d_eps, d_accx, d_accy, d_accz,
                                                       d_mass, G, N);
  }
  // Ensure all GPUs have finished.
  for (int i = 0; i < nGPUs; i++)
  ...
  for (int i = 0; i < nGPUs; i++) // Run kernel on each GPU.
  {
    ...
    cudaSetDevice(i);
    getPressure_Adiabatic_mgpu<<<gridSize, blockSize, 0, streams[i]>>>(beg[i], end[i], d_Typ, d_P, d_rho, d_u, gammah);
  }
  // Ensure all GPUs have finished.
  for (int i = 0; i < nGPUs; i++)
  {
  ...
  for (int i = 0; i < nGPUs; i++) // Run kernel on each GPU.
  {
    ...
    cudaSetDevice(i);
    getCsound_Adiabatic_mgpu<<<gridSize, blockSize, 0, streams[i]>>>(beg[i], end[i], d_Typ, d_csnd, d_u, gammah);
  }
  // Ensure all GPUs have finished.
  for (int i = 0; i < nGPUs; i++)
  {
  ...
  for (int i = 0; i < nGPUs; i++) // Run kernel on each GPU.
  {
    ...
    cudaSetDevice(i);
    div_curlVel_mgpu<<<gridSize, blockSize, 0, streams[i]>>>(beg[i], end[i], d_Typ, d_divV, d_curlV, d_x, d_y, d_z,
                                                             d_vx, d_vy, d_vz, d_rho, d_mass, d_h, N);
  }
  // Ensure all GPUs have finished.
  for (int i = 0; i < nGPUs; i++)
  {
  ...
  for (int i = 0; i < nGPUs; i++) // Run kernel on each GPU.
  {
    ...
    cudaSetDevice(i);
    acc_sph_mgpu<<<gridSize, blockSize, 0, streams[i]>>>(beg[i], end[i], d_Typ, d_x, d_y, d_z,
                                                         d_vx, d_vy, d_vz, d_h, d_csnd, d_rho,
  ...
  for (int i = 0; i < nGPUs; i++) // Run kernel on each GPU.
  {
    ...
    cudaSetDevice(i);
    acc_g_sph_mgpu<<<gridSize, blockSize, 0, streams[i]>>>(beg[i], end[i], d_Typ, d_accx_tot, d_accy_tot, d_accz_tot,
  ...
  for (int i = 0; i < nGPUs; i++) // Run kernel on each GPU.
  {
    ...
    cudaSetDevice(i);
    get_dU_mgpu<<<gridSize, blockSize, 0, streams[i]>>>(beg[i], end[i], d_Typ, d_x, d_y, d_z, d_vx, d_vy, d_vz, d_h, d_csnd, d_rho,
  ...
  for (int i = 0; i < nGPUs; i++) // Run kernel on each GPU.
  {
    ...
    cudaSetDevice(i);
    u_updater_mgpu<<<gridSize, blockSize, 0, streams[i]>>>(beg[i], end[i], d_Typ, d_u, d_dudt, d_utprevious, dt);
  }
  // Ensure all GPUs have finished.
  for (int i = 0; i < nGPUs; i++)
  {
  ...
  while (t < tEnd)
  {
    ...
    for (int i = 0; i < nGPUs; i++) // Run kernel on each GPU.
    {
      ...
      cudaSetDevice(i);
      v_evolve_mgpu<<<gridSize, blockSize, 0, streams[i]>>>(beg[i], end[i], d_Typ, d_vx, d_vy, d_vz, d_accx_tot, d_accy_tot,
                                                            d_accz_tot, dt);
    }
    for (int i = 0; i < nGPUs; i++)
    {
    ...
    for (int i = 0; i < nGPUs; i++) // Run kernel on each GPU.
    {
      ...
      cudaSetDevice(i);
      r_evolve_mgpu<<<gridSize, blockSize, 0, streams[i]>>>(beg[i], end[i], d_Typ, d_x, d_y, d_z, d_vx, d_vy, d_vz, dt);
    }
    for (int i = 0; i < nGPUs; i++)
    {
      cudaSetDevice(i);
    ...
    for (int i = 0; i < nGPUs; i++)
    {
      ...
      cudaSetDevice(i);
      set_eps_of_gas_to_h_mgpu<<<gridSize, blockSize, 0, streams[i]>>>(beg[i], end[i], d_Typ, d_eps, d_h);
    }
    for (int i = 0; i < nGPUs; i++)
    {
      cudaSetDevice(i);
    ...
    for (int i = 0; i < nGPUs; i++)
    {
      ...
      cudaSetDevice(i);
      galaxy_isothermal_potential_mgpu<<<gridSize, blockSize, 0, streams[i]>>>(beg[i], end[i], d_Typ, d_x, d_y, d_z, d_accx_tot,
                                                                               d_accy_tot, d_accz_tot, sigma, G);
    }
    for (int i = 0; i < nGPUs; i++)
    {
      cudaSetDevice(i);
    ...
    auto endxx = std::chrono::high_resolution_clock::now();
    auto elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(endxx - begin);
    ...
    for (int i = 0; i < nGPUs; i++)
    {
      ...
      cudaSetDevice(i);
      dt_array_indiv_dt_mgpu<<<gridSize, blockSize, 0, streams[i]>>>(beg[i], end[i], d_Typ, d_x, d_y, d_z,
                                                                     d_vx, d_vy, d_vz,
                                                                     d_accx, d_accy, d_accz,
    ...
    for (int i = 0; i < nGPUs; i++)
    {
      ...
      cudaSetDevice(i);
      outflow_injector<<<gridSize, blockSize, 0, streams[i]>>>(d_Typ, d_x, d_y, d_z,
                                                               d_vx, d_vy, d_vz,
                                                               d_h, d_eps, d_mass,
