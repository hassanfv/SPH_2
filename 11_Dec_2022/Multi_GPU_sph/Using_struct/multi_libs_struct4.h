...
//*************************************************************************
//*************** Function to save the OUTPUT Snap-Shots II ****************
//*************************************************************************

void saveParticlesToBinary2(const std::string &filename, Particle *particles, int N)
{
  std::ofstream file(filename, std::ios::binary);
  ...
  // Populate the temporary arrays with data from the Particle array
  for (int i = 0; i < N; ++i)
  {
    x[i] = particles[i].x;
    y[i] = particles[i].y;
...
//********* max_finder **********
//*******************************
__global__ void fill_array_max(int *Typ, float *arr, float *result, int N)
{
  ...
  {
    result[i] = 0.0; // Or another small value
  }
}
...
float max_finder(int *Typ, float *arr, int N)
{
  ...
  fill_array_max<<<(N + 255) / 256, 256>>>(Typ, arr, result, N);
  cudaDeviceSynchronize();
  thrust::device_ptr<float> result_ptr(result);
  float max_val = *thrust::max_element(result_ptr, result_ptr + N);
  cudaFree(result);
  return max_val;
}
/*
...
//********* min_finder **********
//*******************************
__global__ void fill_array(int *Typ, float *arr, float *result, int N)
{
  int i = threadIdx.x + blockIdx.x * blockDim.x;
  if (i < N && Typ[i] == 0)
  {
  ...
  {
    result[i] = 1e3; // Or another large value
  }
}
...
float min_finder(int *Typ, float *arr, int N)
{
  float *result;
  cudaMalloc((void **)&result, N * sizeof(float));
  fill_array<<<(N + 255) / 256, 256>>>(Typ, arr, result, N);
  cudaDeviceSynchronize();
  thrust::device_ptr<float> result_ptr(result);
  float min_val = *thrust::min_element(result_ptr, result_ptr + N);
  cudaFree(result);
  return min_val;
}
...
//===============================================
//======= Smoothing Length (MGPUs) struct ======= h_new adopted from eq.31 in Gadget2 Paper
//===============================================
__global__ void smoothing_h_mgpu_struct(int Nbeg, int Nend, Particle *p,
                                        int Ndown, int Nup, float coeff,
                                        float Nngb_f, float dt, int N)
{

  int i = Nbeg + threadIdx.x + blockIdx.x * blockDim.x;

  if ((i < Nend) && (p[i].Type == 0))
  {

    float h_new = 2.0f * (0.5f * p[i].h * (1.0f + pow((Nngb_f / p[i].Nngb_previous), 1.0f / 3.0f)) +
                          1.0f / 3.0f * p[i].h * p[i].divV * dt);
    float h_tmp = h_new;
    int N_iter = 0;
    int k = 0;
    ...
    p[i].Nngb_previous = k;
    p[i].h = 0.01111f; // 0.5 * h_new;
  }
}

//==========================================
//======= getDensity - MGPUs struct ========
//==========================================
__global__ void getDensity_mgpu_struct(int Nbeg, int Nend, Particle *p, int N)
{
...
//========== acc_g - MGPUs - struct ============
//==============================================
__global__ void acc_g_mgpu_struct(int Nbeg, int Nend, Particle *p, float G, int N)
{
  int i = Nbeg + threadIdx.x + blockIdx.x * blockDim.x;

  if ((i < Nend) && ((p[i].Type == 0) || (p[i].Type == 1)))
  {
...
//===================================================
//============= getPressure (Adiabatic) - MGPUs =============
//===================================================
__global__ void getPressure_Adiabatic_mgpu(int Nbeg, int Nend, int *Typ, float *P, float *rho,
                                           float *u, float gammah)
{
...
//===============================================
//============= getCsound (Adiabatic) - MGPUs ===========
//===============================================
__global__ void getCsound_Adiabatic_mgpu(int Nbeg, int Nend, int *Typ, float *csnd,
                                         float *u, float gammah)
{
...
//================== div_curlVel - MGPUs ======================
//=====================================================
__global__ void div_curlVel_mgpu(int Nbeg, int Nend, int *Typ, float *divV, float *curlV,
                                 float *x, float *y, float *z, float *vx, float *vy, float *vz,
                                 float *rho, float *mass, float *h, int N)
{
  int i = Nbeg + threadIdx.x + blockIdx.x * blockDim.x;
...
//====================== acc_sph - MGPUs ============================
//===========================================================
__global__ void acc_sph_mgpu(int Nbeg, int Nend, int *Typ, float *x, float *y, float *z,
...
//====================== acc_tot - MGPUs ============================
//===========================================================
__global__ void acc_g_sph_mgpu(int Nbeg, int Nend, int *Typ,
                               float *acc_totx, float *acc_toty, float *acc_totz,
...
//=================== get_dU - MGPUs ====================
//===============================================
__global__ void get_dU_mgpu(int Nbeg, int Nend, int *Typ, float *x, float *y, float *z,
                            float *vx, float *vy, float *vz,
                            float *h, float *c, float *rho, float *divV, float *curlV,
...
//==================================================
//============== update u & utprevious - MGPUs =============
//==================================================
__global__ void u_updater_mgpu(int Nbeg, int Nend, int *Typ, float *u, float *dudt,
                               float *utprevious, float dt)
{
  int i = Nbeg + threadIdx.x + blockIdx.x * blockDim.x;
...
//===========================================================
//================= velocity evolution - MGPUs ======================
//===========================================================
__global__ void v_evolve_mgpu(int Nbeg, int Nend, int *Typ, float *vx, float *vy, float *vz,
...
//===========================================================
//================= position evolution - MGPUs ======================
//===========================================================
__global__ void r_evolve_mgpu(int Nbeg, int Nend, int *Typ, float *x, float *y, float *z,
                              float *vx, float *vy, float *vz,
                              float dt)
{
...
//==========================================
//========== Set eps of Gas to h - MGPUs ===========
//==========================================
__global__ void set_eps_of_gas_to_h_mgpu(int Nbeg, int Nend, int *Typ, float *eps, float *h)
{
...
//==================================================================================
//========= Isothermal gravitational acceleration (Richings et al - 2018) - MGPUs ===========
//==================================================================================

__global__ void galaxy_isothermal_potential_mgpu(int Nbeg, int Nend, int *Typ, float *x, float *y, float *z,
                                                 float *accx, float *accy, float *accz, float sigma, float G)
{
...
//=================== dt estimation - MGPU =========================
//===========================================================
__global__ void dt_array_indiv_dt_mgpu(int Nbeg, int Nend, int *Typ, float *x, float *y, float *z,
                                       float *vx, float *vy, float *vz,
