...
int main()
{
  ...
  for (int i = 0; i < nGPUs; i++)
  {
    ...
    // Run the kernel only on a portion of the arrays
    acc_g_mgpu_struct<<<gridSize, blockSize>>>(beg[i], end[i], d_p[i], G, N);
  }
  ...
  cout << "T_NG = " << elapsed_acc_g.count() * 1e-9 << endl;
  ...
  cudaMemcpy(p, d_p[0], N * sizeof(Particle), cudaMemcpyDeviceToHost);

  // Specify the output file name
  std::string filenamex = "Test.bin";
  // Save the arrays to binary format
  saveParticlesToBinary2(filenamex, p, N);
