...
//========================================
__global__ void smoothing_h_multi_gpu(int nbeg, int nend, float *x, float *y, float *z,
                                      float *hres, float *hprevious,
                         ...
                         float *divV, float *dh_dt, int N)
{
  ...
  if (i < N)
  {
    ...
    // Note that because h[i] is divided by dh_dt, thus dh_dt[i] actually now has the time unit !
