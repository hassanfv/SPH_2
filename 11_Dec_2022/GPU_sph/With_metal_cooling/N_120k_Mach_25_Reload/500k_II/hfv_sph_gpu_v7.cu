...
int main()
{
  ...
  if (RESTART != "yes")
  {
    string fname = "GPU_IC_DLA_500k.csv"; //!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  if (RESTART == "yes")
  {
    string fname = "RESTART_0.004482.csv"; //!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    string stimex = fname.substr(8, 8);
    ttime = stof(stimex);

    // vector<vector<string>> content; // defined above!
    vector<string> row;
    string line, word;
    ...
    if (file.is_open())
    {
      ...
      cout << endl;
      cout << "The RESTART file is successfully read !" << endl;
    }
    else
      cout << "Could not open the RESTART file\n";
  }
  ...
  float *Nngb_previous, *d_Nngb_previous; // Note that both are floats! check smoothing func. to see why!
  ...
  */
  ...
  float dt = 1e-6; //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! This is only the first time step !!
  ...
  while (t < tEnd)
  {
    ...
    if (!(counter % 50))
    {
      ...
      cudaMemcpy(u, d_u, N * sizeof(float), cudaMemcpyDeviceToHost); // Also used in RESTART output as uprevious.
      cudaMemcpy(uprevious, d_uprevious, N * sizeof(float), cudaMemcpyDeviceToHost);
      cudaMemcpy(dudt, d_dudt, N * sizeof(float), cudaMemcpyDeviceToHost); // used in RESTART output as utprevious.

      cudaMemcpy(hprevious, d_hprevious, N * sizeof(float), cudaMemcpyDeviceToHost); // used in RESTART output!

      ofstream outfile("./Outputs/G-" + to_string(t * 100) + ".csv");
    ...
    if (!(counter % 500))
    {
      ofstream outfileR("./Restart/RESTART_" + to_string(t) + ".csv"); // RESTART file does not need column names!

      for (int i = 0; i < N; i++)
      {
        outfileR << x[i] << "," << y[i] << "," << z[i] << ","
                 << vx[i] << "," << vy[i] << "," << vz[i] << ","
                 << mass[i] << "," << hprevious[i] << "," << eps[i] << ","
                 << u[i] << "," << dudt[i] << endl;
      }
    }
