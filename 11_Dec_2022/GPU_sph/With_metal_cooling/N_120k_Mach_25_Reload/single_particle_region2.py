...
for j in range(0, len(filz), 5):
	...
	y = df['y'].values
	z = df['z'].values
	T = df['u'].values / 4000. * 10000 #
	T = np.array([round(xt, 1) for xt in T])
	...
	cen = np.array([x[jj], y[jj], z[jj]])
	dist = np.sqrt((x-x[jj])**2 + (y-y[jj])**2 + (z-z[jj])**2)
	
	ndist = np.where(dist <= 0.05)[0]
	
	xd = x[ndist]
	yd = y[ndist]
	zd = z[ndist]
	
	print('len ndist = ', len(ndist))
	print('T = ', np.sort(T[ndist]))
	print()
	
	
	ax[1].set_position([0.30, 0.05, 0.21, 0.85])
	ax[1].scatter(x, y, s = 0.01, color = 'black')
	ax[1].scatter(x[jj], y[jj], s = 30, facecolor = 'none', edgecolor = 'blue')
	ax[1].scatter(xd, yd, s = 5, color = 'red')
	...
	ax[2].scatter(y, z, s = 0.01, color = 'black')
	ax[2].scatter(y[jj], z[jj], s = 30, facecolor = 'none', edgecolor = 'blue')
	ax[2].scatter(yd, zd, s = 5, color = 'red')
...
plt.savefig('xsingle_particle.png')
