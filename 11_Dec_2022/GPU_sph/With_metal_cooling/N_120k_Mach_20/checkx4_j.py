...
'''
# Adding leading 0 to the name if t < 10 !
for j in range(len(filz)):

	oldName = filz[j]

	fileName_Num = filz[j].split('/')[-1][2:-4]	
	tt = float(filz[j].split('/')[-1][2:-4])

	if tt < 10:
		
		fileName_Num = '0' + fileName_Num
		newName = NEW_PATH + 'G-' + fileName_Num + '.csv'
...
#filz = np.sort(glob.glob(OLD_PATH + '*.csv'))
