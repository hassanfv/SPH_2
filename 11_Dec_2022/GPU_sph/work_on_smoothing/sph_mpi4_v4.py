...
with open('Evrard_5616.pkl', 'rb') as f:   # !!!!!! Change epsilon
    res = pickle.load(f)
...
Nngb_previous = 64 + np.zeros(N)

local_h = smoothing_fast_new_mpi3(nbeg, nend, r, h, Nngb_previous, divV, dt)

if rank == 0:
	h, Nngb_previous = local_h
	for i in range(1, nCPUs):
		htmp, Nngb_previous_tmp = comm.recv(source = i)
		h = np.concatenate((h, htmp))
		Nngb_previous = np.concatenate((Nngb_previous, Nngb_previous_tmp))
else:
	comm.send(local_h, dest = 0)

h = comm.bcast(h, root = 0)
Nngb_previous = comm.bcast(Nngb_previous, root = 0)
#----------------------
