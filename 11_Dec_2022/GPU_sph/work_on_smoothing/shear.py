...
def div_curlVel_mpi(nbeg, nend, pos, v, rho, m, h):
	...
	for i in range(nbeg, nend):
		...
		divV[i-nbeg] = s # np.abs(s)  # Note that we have one divVel for each particle.
...
def getAcc_sph_shear(pos, v, rho, P, c, h, m, divV, curlV, alpha): # shear viscosity-corrected
	...
	for i in range(N):
		...
		for j in range(N):
			...
			fi = np.abs(divV[i])/(np.abs(divV[i]) + curlV[i] + 0.0001*c[i]/h[i])
			fj = np.abs(divV[i])/(np.abs(divV[i]) + curlV[j] + 0.0001*c[j]/h[j])
			fij = 0.5 * (fi + fj)
			PIij = fij * PIij
