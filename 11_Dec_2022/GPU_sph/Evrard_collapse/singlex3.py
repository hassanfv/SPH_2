...
#u = dictx['u'].flatten()
P = 0.0 + np.zeros(rx.shape[0]) #(gamma - 1.) * u * rho
...
for i in range(NN-1):
	...
	nt = np.where((rr >= grid[i]) & (rr < grid[i+1]))[0]
	
	if len(nt) > 0:
		r_tmp = rr[nt]
		v_tmp = vv[nt]
		
		res.append([np.median(r_tmp), np.median(v_tmp)])
...
ax[0].scatter(res[:, 0], res[:, 1], s = 12, alpha = 1.0, color = 'k', label='This work')
