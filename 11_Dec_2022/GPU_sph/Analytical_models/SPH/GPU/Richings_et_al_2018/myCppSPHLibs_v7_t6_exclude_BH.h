...
// Functions required for the Richings et al - 2018 simulation added. (19 May 2023).
...
//========================================
//========== Smoothing Length EX BH ============ 
//========================================
__global__ void smoothing_h_exBH(float *x, float *y, float *z, float *hres, float *hprevious,
                            int N, int Ndown, int Nup, float coeff,
                            float Nngb_f, float *Nngb_previous, float *divV, float dt)
{
...
//==========================================
//============ getDensity Exclude BH ==================
//==========================================
__global__ void getDensity_exBH(float *x, float *y, float *z, float *mass,
                                float *rho, float *h, int N)
{

  int i = threadIdx.x + blockIdx.x * blockDim.x;

  if ((i > 0) && (i < N)) // (i > 1) excludes the first particle from calculation as it is the BH!
  {
    ...
    for (int j = 1; j < N; j++) // Note that j starts with 1 to skip BH which is the first element!
    {
      dx = x[i] - x[j];
      dy = y[i] - y[j];
      dz = z[i] - z[j];
...
//================== div_curlVel - EX BH ======================
//=====================================================
__global__ void div_curlVel_exBH(float *divV, float *curlV, float *x, float *y, float *z,
                            float *vx, float *vy, float *vz, float *rho, float *mass,
                            float *h, int N)
{
  int i = threadIdx.x + blockIdx.x * blockDim.x;
...
//====================== acc_sph ============================
//===========================================================
__global__ void acc_sph_exBH(float *x, float *y, float *z, float *vx, float *vy, float *vz,
                        float *h, float *c, float *rho, float *divV, float *curlV,
                        float *mass, float *P, float *ax, float *ay, float *az,
                        float visc_alpha, int N)
{
...
//====================== acc_tot EX BH ============================
//===========================================================
__global__ void acc_g_sph_exBH(float *acc_totx, float *acc_toty, float *acc_totz,
                          float *acc_gx, float *acc_gy, float *acc_gz,
                          float *acc_sphx, float *acc_sphy, float *acc_sphz,
                          int N)
{
...
//===========================================================
//================= position evolution EX BH ======================
//===========================================================
__global__ void r_evolve_exBH(float *x, float *y, float *z,
                         float *vx, float *vy, float *vz,
                         float dt, int N)
{
...
//=================== dt estimation EX BH =========================
//===========================================================
__global__ void dt_array_indiv_dt_exBH(float *x, float *y, float *z,
                                  float *vx, float *vy, float *vz,
                                  float *accx, float *accy, float *accz,
                                  float *accx_tot, float *accy_tot, float *accz_tot,
                                  float *h, float *c, float *dt_particles,
                                  ...
                                  float visc_alpha, float *eps, int N)
{
  ...
  dh_dt[0] = 1000.0f; // using large values for BH particle so that it skipps!!!!!
}
...
//=================== get_dU EX BH ====================
//===============================================
__global__ void get_dU_exBH(float *x, float *y, float *z, float *vx, float *vy, float *vz,
                       float *h, float *c, float *rho, float *divV, float *curlV,
                       float *mass, float *P, float *dudt,
                       float visc_alpha, int N)
{
...
//===============================================
//========= update u (Used only once!) EX BH ==========
//===============================================
__global__ void u_updater1_exBH(float *u, float *dudt, float dt, int N)
{
  int i = threadIdx.x + blockIdx.x * blockDim.x;
...
//===============================================
//============ update u_ut_previous EX BH =============
//===============================================
__global__ void u_ut_previous_updater_exBH(float *u, float *dudt, float *uprevious,
                                      float *utprevious, int N)
{
  int i = threadIdx.x + blockIdx.x * blockDim.x;
...
//===============================================
//============= update u (Main one) EX BH =============
//===============================================
__global__ void u_updater_main_exBH(float *u, float *dudt, float *uprevious,
                               float *utprevious, float dt, int N)
{
  int i = threadIdx.x + blockIdx.x * blockDim.x;
...
//===================================================
//============= getPressure (Adiabatic) - EX_BH =============
//===================================================
__global__ void getPressure_Adiabatic_exBH(float *P, float *rho, float *u, float gammah, int N)
{
...
//===============================================
//============= getCsound (Adiabatic) - EX BH ===========
//===============================================
__global__ void getCsound_Adiabatic_exBH(float *csnd, float *u, float gammah, int N)
{
...
//==========================================
//============ getDensity (Single particle) ==================
//==========================================
__device__ float getDensity_single_particle(float *x, float *y, float *z, float xp, float yp, float zp,
                                            float *mass, float *h, int N)
{
  ...
  return ss; // ss is the gas density at (xp, yp, zp).
}
...
/*
//========================================
//======= Smoothing Length (Single Particle) Device Function =========
//========================================

__device__ float smoothing_h_single(float *x, float *y, float *z, float hBH,
                                    int N, int Ndown, int Nup, float coeff)
{
...
//=============== AGN Wind Injection ============
//===============================================
__global__ void agn_wind_injection(float *x, float *y, float *z, float *vx, float *vy, float *vz, 
                                   float *u, float *mass, float *h, float *epsilon, float M_dot_in,
                                   float v_in, float dt, float leftover_mass, float m_sph_high_res,
                                   float u_for_10K_Temp, int N)

{
  ...
  if (i == 0)
  {
    ...
    float xt, yt, zt, rt;
    float rho_p, h_p;
    float no_of_added_particles = 0.0f; // This will be used to update the main N !! We will convert it to int in the main program!

    //--- used for random number generation ---
    // Initiate curandState
    curandState state;
    ...
    //M_dot_in = Tou_in * L_AGN / clight / v_in;
    
    float mass_to_inject = M_dot_in * dt;
    mass_to_inject += leftover_mass;
  
    while (mass_to_inject >= 2.0f * m_sph_high_res)
    {
      
      xt = curand_uniform(&state);
      yt = curand_uniform(&state);
      zt = curand_uniform(&state);
      
      rt = sqrt(xt * xt + yt * yt + zt * zt);
      
      if (rt > 1.0f)
        continue; // if rt < 1.0, it will go back to the begining of the while loop!! VERY interesting !!!!
      
      // Spawning 2 particles in the opposite direction!
      x[j] = xt;
      y[j] = yt;
      z[j] = zt;
      ...
      // Assigning opposite radial direction velocity to the 2 particles!
      vx[j] = xt / rt * v_in;
      vy[j] = yt / rt * v_in;
      vz[j] = zt / rt * v_in;
      ...
      // assigning mass to the 2 new particles in the main mass array, i.e. m.
      mass[j] = m_sph_high_res;
      mass[j+1] = m_sph_high_res;
      
      // assigning internal energy u to the 2 new particles.
      //utmp = (3.0f/2.0f) * k_B * T_gas / mu / m_p / Unit_u_in_cgs;
      u[j] = u_for_10K_Temp;
      u[j+1] = u_for_10K_Temp;
      
      // assigning smoothing length h and gravitational softening epsilon to the 2 new particles.
      rho_p = getDensity_single_particle(x, y, z, xt, yt, zt, mass, h, N);
      h_p = pow(m_sph_high_res / rho_p, 1.0f/3.0f); // see Richings et al - 2018, page 3, right-hand colum, first line!
      ...
      // update mass_to_inject
      mass_to_inject -= 2.0f * m_sph_high_res;
      
      no_of_added_particles += 2.0f;
      
      j = j + 2;
      
      leftover_mass = mass_to_inject;
      u[j] = leftover_mass; // It is just a clever idea to have access to the leftover_mass in the main code. We will assign the leftover_mass to
                            // u[j] (note that j = j+2 so it will always indicate a place that is not used currently!). It should be clear to you !
      
      u[0] = no_of_added_particles; // Use BH particle space to save this info!!!
