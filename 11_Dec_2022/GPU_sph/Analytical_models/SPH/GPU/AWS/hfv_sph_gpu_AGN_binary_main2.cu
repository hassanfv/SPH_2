...
int main()
{
  ...
  std::ifstream file("IC_AGN_Evolved_0.00Gyr_200k.bin", std::ios::binary);
  if (!file) {
  ...
  file.open("IC_AGN_Evolved_0.00Gyr_200k.bin", std::ios::binary);
  if (file) {
  ...
  cudaMalloc(&d_dt_particles, NG * sizeof(float));
  ...
  */
  ...
  cudaMemcpy(d_dt_particles, dt_particles, NG * sizeof(float), cudaMemcpyHostToDevice);
  ...
  while (t < tEnd)
  {
    ...
    */
    ...
    u_AGN_energy_velocity_injection<<<gridSize, blockSize>>>(d_u, d_weightsBH, d_x, d_y, d_z,
                                                             d_vx, d_vy, d_vz, eps_AGN,
                                                             L_Edd, dt, NG);
    cudaDeviceSynchronize();
    ...
    cudaMemcpy(dt_particles, d_dt_particles, NG * sizeof(float), cudaMemcpyDeviceToHost);

    dt = min_finder(dt_particles, NG);

    dt_max = max_finder(dt_particles, NG);
