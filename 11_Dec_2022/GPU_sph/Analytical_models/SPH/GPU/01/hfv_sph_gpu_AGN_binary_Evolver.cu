...
int main()
{
  ...
  std::ifstream file("IC_AGN_351k.bin", std::ios::binary);
  if (!file) {
  ...
  file.open("IC_AGN_351k.bin", std::ios::binary);
  if (file) {
  ...
  //cudaMalloc(&d_NGroupz, NG * sizeof(int));
  ...
  */
  ...
  // Initialize x, y, and z on the Host (All particles, i.e. Gas + DM).
  for (int i = 0; i < N; i++)
  {
    x[i] = xvec[i];
    y[i] = yvec[i];
