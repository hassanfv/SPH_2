...
N_start = 200000 # Play with this to get the desired number of particles!
...
tff = np.sqrt(3*np.pi/(32*grav_const_in_cgs*rho_0))                   # The free-fall time = 3.4e4 yr
...
u = 5000.0 + np.zeros(Npart) # in code unit # I used check4.py code here to find this value!
u_in_cgs = u * Unit_u_in_cgs # in cgs

T = convert_u_to_temp_h(u_in_cgs[0], nH_0, XH)

print(f'T = {T:.2f} K which corresponds to u (code unit) = {u[0]} and to u (cgs) = {u_in_cgs[0]:.1f}')
