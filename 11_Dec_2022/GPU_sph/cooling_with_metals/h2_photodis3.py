...
L_912 = 1e13 * 3.846e33 # 3.25e42 # erg/s/A
L_912 = flamb_to_fneu(L_912, 912.0) # L is now in erg/s/Hz
dist = 1e3 # 500.0 # pc
dist_cm = pc_to_cm(dist)
J912QSO = 1.0/(4. * np.pi) * L_912 / (4. * np.pi * dist_cm**2) # J at neu = 912 A or 1 ryd.
...
neu = [x/hplanck for x in h_nu_erg]

delta_nu = neu[1] - neu[0]

x = [eV_to_ryd(neu_to_eV(neu_t)) for neu_t in neu] # x is in rydberg !
...
Jnu = [J_nu_func(xt, tau_0, alpha, fQ, J_0) for xt in x] # xt must be in in ryd
...
x = [eV_to_ryd(neu_to_eV(neu_t)) for neu_t in neuH0] # x is in rydberg !
JnuH0 = [J_nu_func(xt, tau_0, alpha, fQ, J_0) for xt in x] # xt must be in in ryd

plt.scatter(x, JnuH0, s = 10)
plt.yscale('log')
plt.show()

x = [eV_to_ryd(neu_to_eV(neu_t)) for neu_t in neuHe0] # x is in rydberg !
JnuHe0 = [J_nu_func(xt, tau_0, alpha, fQ, J_0) for xt in x] # xt must be in in ryd
