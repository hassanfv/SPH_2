...
NSample = 600000 # The desired number of particles to be included in ONE cloud in the IC.
NSample = int(NSample)

with open('Main_IC_Grid_606k_b_0.4_Mach_10.pkl', 'rb') as f:
	data = pickle.load(f)

# These contain data for both colliding clouds!
rr = data['r']

if NSample > rr.shape[0]:
	print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
	print('WARNING !!! NSample is greater than rr.shape !!! Please change NSample!!!!')
	print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
	exit(0)
...
num = str(int(NSample/1000)) # Used for out put file name.

with open('IC_Kitsonias_' + num + 'k_RAND.pkl', 'wb') as f:
	pickle.dump(dictx, f)
#----------------------------
...
epsilon = 0.005 + np.zeros(len(x))

dictx = {'x': x, 'y': y, 'z': z, 'vx': vx, 'vy': vy, 'vz': vz, 'm': m, 'h': np.round(h, 6), 'eps': epsilon}

df = pd.DataFrame(dictx)

df.to_csv('GPU_IC_Kitsonias' + '_' + num + 'k_RAND.csv', index = False, header = False)
