...
// The difference with the previous version is that here the output file also contains the velocity components.
...
int main(){
  ...
  // Reading Hydra file.
  string fname = "GPU_IC_Arrega_180k.csv"; //!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  //                                     gammah, N);
  getPressure_Arreaga<<<gridSize, blockSize>>>(d_P, d_rho, UnitDensity_in_cgs, 
  ...
  //                                     gammah, N);
  getCsound_Arreaga<<<gridSize, blockSize>>>(d_csnd, d_rho, UnitDensity_in_cgs,
  ...
  while(t < tEnd){    
    ...
    if(!(counter % 5)){
      ...
      outfile << "x" << "," << "y" << "," << "z" << "," << "vx" << "," << "vy" << ","
              << "vz" << "," << "h" << "," << "rho" << endl; // this will be the header !
