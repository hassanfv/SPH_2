...
int main()
{
  ...
  for (int i = 0; i < nGPUs; ++i) {
    for (int j = 0; j < nGPUs; ++j) {
      if (i != j) {  // Don't check for self-access
        CHECK_CUDA(cudaDeviceCanAccessPeer(&canAccessPeer, i, j));
        
        if (canAccessPeer) {
          printf("Device %d can access memory of Device %d\n", i, j);
          
          // Enable peer-to-peer access
          CHECK_CUDA(cudaSetDevice(i));
          CHECK_CUDA(cudaDeviceEnablePeerAccess(j, 0));
        } else {
          printf("Device %d cannot access memory of Device %d\n", i, j);
        }
      }
    }
  }
...
for (int i = 0; i < nGPUs; i++)
  ...
  for (int i = 1; i < nGPUs; i++)
  {
    ...
    CHECK_CUDA(cudaMemcpyPeer(d_h[0] + NN, 0, d_h[i] + NN, i, MLen[i] * sizeof(float)));
    CHECK_CUDA(cudaMemcpyPeer(d_Nngb_previous[0] + NN, 0, d_Nngb_previous[i] + NN, i, MLen[i] * sizeof(float)));
  ...
  {
    CHECK_CUDA(cudaMemcpyPeer(d_h[i], i, d_h[0], 0, N * sizeof(float)));
    CHECK_CUDA(cudaMemcpyPeer(d_Nngb_previous[i], i, d_Nngb_previous[0], 0, N * sizeof(float)));
  }
  ...
  for (int i = 1; i < nGPUs; i++)
  {
    ...
    CHECK_CUDA(cudaMemcpyPeer(d_rho[0] + NN, 0, d_rho[i] + NN, i, MLen[i] * sizeof(float)));
  ...
  {
    CHECK_CUDA(cudaMemcpyPeer(d_rho[i], i, d_rho[0], 0, N * sizeof(float)));
  }
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  //!!!!!!!!!!!!!!!!!!! END of getDensity !!!!!!!!!!!!!!!!!!!
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  for (int i = 1; i < nGPUs; i++)
  {
    ...
    CHECK_CUDA(cudaMemcpyPeer(d_accx[0] + NN, 0, d_accx[i] + NN, i, MLen[i] * sizeof(float)));
    CHECK_CUDA(cudaMemcpyPeer(d_accy[0] + NN, 0, d_accy[i] + NN, i, MLen[i] * sizeof(float)));
    CHECK_CUDA(cudaMemcpyPeer(d_accz[0] + NN, 0, d_accz[i] + NN, i, MLen[i] * sizeof(float)));
  ...
  {
    CHECK_CUDA(cudaMemcpyPeer(d_accx[i], i, d_accx[0], 0, N * sizeof(float)));
    CHECK_CUDA(cudaMemcpyPeer(d_accy[i], i, d_accy[0], 0, N * sizeof(float)));
    CHECK_CUDA(cudaMemcpyPeer(d_accz[i], i, d_accz[0], 0, N * sizeof(float)));
  }
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  //!!!!!!!!!!!!!!! END OF acc_g !!!!!!!!!!!!!!!!!!!
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  for (int i = 1; i < nGPUs; i++)
  {
    ...
    CHECK_CUDA(cudaMemcpyPeer(d_P[0] + NN, 0, d_P[i] + NN, i, MLen[i] * sizeof(float)));
  ...
  for (int i = 1; i < nGPUs; i++) // Note that GPU 0 already hass the data!
  {
    CHECK_CUDA(cudaMemcpyPeer(d_P[i], i, d_P[0], 0, N * sizeof(float)));
  }
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  //!!!!!!!!!!!!!!! END OF getPressure !!!!!!!!!!!!!!!!!!!
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  for (int i = 1; i < nGPUs; i++)
  {
    ...
    CHECK_CUDA(cudaMemcpyPeer(d_csnd[0] + NN, 0, d_csnd[i] + NN, i, MLen[i] * sizeof(float)));

    CHECK_CUDA(NN = NN + MLen[i]);
  }

  // Copy results back to each device for next iteration
  for (int i = 1; i < nGPUs; i++) // Note that GPU 0 already hass the data!
  {
    CHECK_CUDA(cudaMemcpyPeer(d_csnd[i], i, d_csnd[0], 0, N * sizeof(float)));
  }
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  //!!!!!!!!!!!!!!! END OF getCsound !!!!!!!!!!!!!!!!!!!
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  for (int i = 1; i < nGPUs; i++)
  {
    ...
    CHECK_CUDA(cudaMemcpyPeer(d_divV[0] + NN, 0, d_divV[i] + NN, i, MLen[i] * sizeof(float)));
    CHECK_CUDA(cudaMemcpyPeer(d_curlV[0] + NN, 0, d_curlV[i] + NN, i, MLen[i] * sizeof(float)));
  ...
  for (int i = 1; i < nGPUs; i++) // Note that GPU 0 already hass the data!
  {
    CHECK_CUDA(cudaMemcpyPeer(d_divV[i], i, d_divV[0], 0, N * sizeof(float)));
    CHECK_CUDA(cudaMemcpyPeer(d_curlV[i], i, d_curlV[0], 0, N * sizeof(float)));
  }
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  //!!!!!!!!!!!!!!! END OF div_curlV !!!!!!!!!!!!!!!!!!!
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  for (int i = 1; i < nGPUs; i++)
  {
    ...
    CHECK_CUDA(cudaMemcpyPeer(d_accx_sph[0] + NN, 0, d_accx_sph[i] + NN, i, MLen[i] * sizeof(float)));
    CHECK_CUDA(cudaMemcpyPeer(d_accy_sph[0] + NN, 0, d_accy_sph[i] + NN, i, MLen[i] * sizeof(float)));
    CHECK_CUDA(cudaMemcpyPeer(d_accz_sph[0] + NN, 0, d_accz_sph[i] + NN, i, MLen[i] * sizeof(float)));
  ...
  for (int i = 1; i < nGPUs; i++) // Note that GPU 0 already hass the data!
  {
    CHECK_CUDA(cudaMemcpyPeer(d_accx_sph[i], i, d_accx_sph[0], 0, N * sizeof(float)));
    CHECK_CUDA(cudaMemcpyPeer(d_accy_sph[i], i, d_accy_sph[0], 0, N * sizeof(float)));
    CHECK_CUDA(cudaMemcpyPeer(d_accz_sph[i], i, d_accz_sph[0], 0, N * sizeof(float)));
  }
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  //!!!!!!!!!!!!!!! END OF acc_sph !!!!!!!!!!!!!!!!!!!
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  for (int i = 1; i < nGPUs; i++)
  {
    ...
    CHECK_CUDA(cudaMemcpyPeer(d_accx_tot[0] + NN, 0, d_accx_tot[i] + NN, i, MLen[i] * sizeof(float)));
    CHECK_CUDA(cudaMemcpyPeer(d_accy_tot[0] + NN, 0, d_accy_tot[i] + NN, i, MLen[i] * sizeof(float)));
    CHECK_CUDA(cudaMemcpyPeer(d_accz_tot[0] + NN, 0, d_accz_tot[i] + NN, i, MLen[i] * sizeof(float)));
  ...
  for (int i = 1; i < nGPUs; i++) // Note that GPU 0 already hass the data!
  {
    CHECK_CUDA(cudaMemcpyPeer(d_accx_tot[i], i, d_accx_tot[0], 0, N * sizeof(float)));
    CHECK_CUDA(cudaMemcpyPeer(d_accy_tot[i], i, d_accy_tot[0], 0, N * sizeof(float)));
    CHECK_CUDA(cudaMemcpyPeer(d_accz_tot[i], i, d_accz_tot[0], 0, N * sizeof(float)));
  }
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  //!!!!!!!!!!!!!!! END OF acc_tot !!!!!!!!!!!!!!!!!!!
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  for (int i = 1; i < nGPUs; i++)
  {
    ...
    CHECK_CUDA(cudaMemcpyPeer(d_dudt[0] + NN, 0, d_dudt[i] + NN, i, MLen[i] * sizeof(float)));
  ...
  for (int i = 1; i < nGPUs; i++) // Note that GPU 0 already hass the data!
  {
    CHECK_CUDA(cudaMemcpyPeer(d_dudt[i], i, d_dudt[0], 0, N * sizeof(float)));
  }
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  //!!!!!!!!!!!!!!! END OF du_dt !!!!!!!!!!!!!!!!!!!
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  for (int i = 1; i < nGPUs; i++)
  {
    ...
    CHECK_CUDA(cudaMemcpyPeer(d_u[0] + NN, 0, d_u[i] + NN, i, MLen[i] * sizeof(float)));
    CHECK_CUDA(cudaMemcpyPeer(d_utprevious[0] + NN, 0, d_utprevious[i] + NN, i, MLen[i] * sizeof(float)));
  ...
  for (int i = 1; i < nGPUs; i++) // Note that GPU 0 already hass the data!
  {
    CHECK_CUDA(cudaMemcpyPeer(d_u[i], i, d_u[0], 0, N * sizeof(float)));
    CHECK_CUDA(cudaMemcpyPeer(d_utprevious[i], i, d_utprevious[0], 0, N * sizeof(float)));
  }
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  //!!!!!!!!!!!!!!! END OF u evolution !!!!!!!!!!!!!!!!!!!
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  CHECK_CUDA(cudaSetDevice(0)); // Set the first GPU as active ===> Take note of "d_leftover_mass" that is only recognizable in GPU 0 !

  float leftover_mass = 0.0f;
  float *d_leftover_mass;
  cudaMalloc((void **)&d_leftover_mass, sizeof(float));
  CHECK_CUDA(cudaMemcpy(d_leftover_mass, &leftover_mass, sizeof(float), cudaMemcpyHostToDevice));
  ...
  while (t < tEnd)
  {
    ...
    for (int i = 1; i < nGPUs; i++)
    {
      ...
      CHECK_CUDA(cudaMemcpyPeer(d_vx[0] + NN, 0, d_vx[i] + NN, i, MLen[i] * sizeof(float)));
      CHECK_CUDA(cudaMemcpyPeer(d_vy[0] + NN, 0, d_vy[i] + NN, i, MLen[i] * sizeof(float)));
      CHECK_CUDA(cudaMemcpyPeer(d_vz[0] + NN, 0, d_vz[i] + NN, i, MLen[i] * sizeof(float)));
    ...
    for (int i = 1; i < nGPUs; i++) // Note that GPU 0 already hass the data!
    {
      CHECK_CUDA(cudaMemcpyPeer(d_vx[i], i, d_vx[0], 0, N * sizeof(float)));
      CHECK_CUDA(cudaMemcpyPeer(d_vy[i], i, d_vy[0], 0, N * sizeof(float)));
      CHECK_CUDA(cudaMemcpyPeer(d_vz[i], i, d_vz[0], 0, N * sizeof(float)));
    }
    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    for (int i = 0; i < nGPUs; i++)
    {
      ...
      r_evolve_mgpu<<<gridSize, blockSize>>>(beg[i], end[i], d_Typ[i], d_x[i], d_y[i], d_z[i],
                                             d_vx[i], d_vy[i], d_vz[i], dt, ndx_BH);
    }
    ...
    for (int i = 1; i < nGPUs; i++)
    {
      ...
      CHECK_CUDA(cudaMemcpyPeer(d_x[0] + NN, 0, d_x[i] + NN, i, MLen[i] * sizeof(float)));
      CHECK_CUDA(cudaMemcpyPeer(d_y[0] + NN, 0, d_y[i] + NN, i, MLen[i] * sizeof(float)));
      CHECK_CUDA(cudaMemcpyPeer(d_z[0] + NN, 0, d_z[i] + NN, i, MLen[i] * sizeof(float)));
    ...
    for (int i = 1; i < nGPUs; i++) // Note that GPU 0 already has the data!
    {
      CHECK_CUDA(cudaMemcpyPeer(d_x[i], i, d_x[0], 0, N * sizeof(float)));
      CHECK_CUDA(cudaMemcpyPeer(d_y[i], i, d_y[0], 0, N * sizeof(float)));
      CHECK_CUDA(cudaMemcpyPeer(d_z[i], i, d_z[0], 0, N * sizeof(float)));
    }
    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    for (int i = 1; i < nGPUs; i++)
    {
      ...
      CHECK_CUDA(cudaMemcpyPeer(d_eps[0] + NN, 0, d_eps[i] + NN, i, MLen[i] * sizeof(float)));
    ...
    for (int i = 1; i < nGPUs; i++) // Note that GPU 0 already hass the data!
    {
      CHECK_CUDA(cudaMemcpyPeer(d_eps[i], i, d_eps[0], 0, N * sizeof(float)));
    }
    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    //!!!!!!!!!!!!!!!!!!! START of getDensity !!!!!!!!!!!!!!!!!
    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    auto T_rho = std::chrono::high_resolution_clock::now();
    for (int i = 0; i < nGPUs; i++)
    {
      CHECK_CUDA(cudaSetDevice(i));
    ...
    auto end_rho = std::chrono::high_resolution_clock::now();
    auto elapsed_rho = std::chrono::duration_cast<std::chrono::nanoseconds>(end_rho - T_rho);
    cout << "T_rho = " << elapsed_rho.count() * 1e-9 << endl;
    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    //!!!!!!!!!!!!!!!!!!! END of getDensity !!!!!!!!!!!!!!!!!!!
    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    CHECK_CUDA(cudaSetDevice(0)); // !!!!! Heating and Cooling are performed ONLY by the first GPU, i.e., GPU 0 !!!
    ...
    for (int i = 1; i < nGPUs; i++) // Note that GPU 0 already hass the data!
    {
      CHECK_CUDA(cudaMemcpyPeer(d_u[i], i, d_u[0], 0, N * sizeof(float))); // Only u is updated during heating-cooling process!
    }
    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    CHECK_CUDA(cudaMemcpy(rho, d_rho[0], N * sizeof(float), cudaMemcpyDeviceToHost));
    for (int i = 0; i < 5; i++)
    {
      cout << "AAA = " << rho[i] << endl;
    }
    ...
    //if (counter > -1)
    {
      CHECK_CUDA(cudaMemcpy(Typ, d_Typ[0], N * sizeof(int), cudaMemcpyDeviceToHost));

      CHECK_CUDA(cudaMemcpy(x, d_x[0], N * sizeof(float), cudaMemcpyDeviceToHost));
      CHECK_CUDA(cudaMemcpy(y, d_y[0], N * sizeof(float), cudaMemcpyDeviceToHost));
      CHECK_CUDA(cudaMemcpy(z, d_z[0], N * sizeof(float), cudaMemcpyDeviceToHost));

      CHECK_CUDA(cudaMemcpy(vx, d_vx[0], N * sizeof(float), cudaMemcpyDeviceToHost));
      CHECK_CUDA(cudaMemcpy(vy, d_vy[0], N * sizeof(float), cudaMemcpyDeviceToHost));
      CHECK_CUDA(cudaMemcpy(vz, d_vz[0], N * sizeof(float), cudaMemcpyDeviceToHost));

      CHECK_CUDA(cudaMemcpy(rho, d_rho[0], N * sizeof(float), cudaMemcpyDeviceToHost));
      CHECK_CUDA(cudaMemcpy(h, d_h[0], N * sizeof(float), cudaMemcpyDeviceToHost));

      CHECK_CUDA(cudaMemcpy(u, d_u[0], N * sizeof(float), cudaMemcpyDeviceToHost));
      
      CHECK_CUDA(cudaMemcpy(mass, d_mass[0], N * sizeof(float), cudaMemcpyDeviceToHost));

      CHECK_CUDA(cudaMemcpy(ionFrac, d_ionFrac, N_ionFrac * sizeof(float), cudaMemcpyDeviceToHost));
    ...
    outflow_injector2<<<gridSize, blockSize>>>(d_Typ[0], d_x[0], d_y[0], d_z[0],
    ...
    for (int i = 1; i < nGPUs; i++) // Note that GPU 0 already has the data!
    {
      CHECK_CUDA(cudaMemcpyPeer(d_Typ[i], i, d_Typ[0], 0, N * sizeof(int)));
      ...
      CHECK_CUDA(cudaMemcpyPeer(d_eps[i], i, d_eps[0], 0, N * sizeof(float)));
      CHECK_CUDA(cudaMemcpyPeer(d_mass[i], i, d_mass[0], 0, N * sizeof(float)));
      CHECK_CUDA(cudaMemcpyPeer(d_Nngb_previous[i], i, d_Nngb_previous[0], 0, N * sizeof(float)));
      CHECK_CUDA(cudaMemcpyPeer(d_u[i], i, d_u[0], 0, N * sizeof(float)));
    }
    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
