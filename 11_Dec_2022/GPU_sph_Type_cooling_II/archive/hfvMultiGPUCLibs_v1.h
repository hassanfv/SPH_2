...
float max_finder(int *Typ, float *arr, int N)
{
    ...
    err = cudaMalloc((void **)&result, N * sizeof(float));
    if (err != cudaSuccess)
    {
    ...
    if (err != cudaSuccess)
    {
        fprintf(stderr, "Failed to launch fill_array_max kernel (error code %s)!\n", cudaGetErrorString(err));
        cudaFree(result);
        exit(EXIT_FAILURE);
    }
...
//========================================
//======= Smoothing Length (MGPUs) ======= Updated: 28 Jan 2023. h_new adopted from eq.31 in Gadget2 Paper
//========================================
__global__ void smoothing_h_mgpu(int Nbeg, int Nend, int *Typ, float *x, float *y, float *z, float *h,
                                 int Ndown, int Nup, float coeff,
                                 float Nngb_f, float *Nngb_previous, float *divV, float dt, int N)
{
