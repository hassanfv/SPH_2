...
//--------------------------------------------
  ...
  float min_val = 1e22; // used large value for the start!
  ...
  while (t < tEnd)
  {
    ...
    auto elapsed_dt = std::chrono::duration_cast<std::chrono::nanoseconds>(end_dt - T_dt);
    cout << "T_dt = " << elapsed_dt.count() * 1e-9 << endl;
    ...
    auto elapsed_copy = std::chrono::duration_cast<std::chrono::nanoseconds>(end_copy - T_copy);
    cout << "T_copy = " << elapsed_copy.count() * 1e-9 << endl;
    
    t += dt;
    
    min_val = 1e22; // used large value for the start! I did not want to use arr[0] as it could be 0.0 itself!
    for (int i = 0; i < N; i++)
    {
      if (Typ[i] == 0)
      {
        if (dt_particles[i] <= min_val)
        {
          min_val = dt_particles[i];
        }
      }
    }
