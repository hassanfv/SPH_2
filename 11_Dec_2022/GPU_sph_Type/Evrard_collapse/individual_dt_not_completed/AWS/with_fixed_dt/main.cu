...
int main()
{
  ...
  float dt = 1e-5; //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! This is only the first time step !!
  ...
  update_t_next<<<gridSize, blockSize>>>(d_Typ, d_t_last, d_t_next, d_dt_particles, t, N);
  cudaDeviceSynchronize();

  update_t_last_activeId<<<gridSize, blockSize>>>(d_Typ, d_t_last, d_t_next, d_activeId, d_dt_particles,
                                                  t, dt, N);
  cudaDeviceSynchronize();
  ...
  // who_is_active<<<gridSize, blockSize>>>(d_Typ, d_activeId, d_t_next, dt, t, N);
  // cudaDeviceSynchronize();

  // update_t_next<<<gridSize, blockSize>>>(d_Typ, d_t_last, d_t_next, d_activeId, d_dt_particles, t, N);
  // cudaDeviceSynchronize();
