...
def read_arrays_from_binary(filename):
    ...
    with open(filename, 'rb') as file:
        ...
        Typ = np.frombuffer(file.read(N * 4), dtype=np.int32)
        ...
        rho = np.frombuffer(file.read(N * 4), dtype=np.float32)
        h = np.frombuffer(file.read(N * 4), dtype=np.float32)
        u = np.frombuffer(file.read(N * 4), dtype=np.float32)
        m = np.frombuffer(file.read(N * 4), dtype=np.float32)

    return x, y, z, vx, vy, vz, rho, h, u, m, Typ, N

# Specify the input file name
filename = 'G-0.793012.bin'
...
#  d = (rx[i]*rx[i] + ry[i]*ry[i] + rz[i]*rz[i])
#  vv[i] = (rx[i]*vx[i] + ry[i]*vy[i] + rz[i]*vz[i]) / d
