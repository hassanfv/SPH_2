...
N_particles = 10000 # In the current simulation all these particles are gas particle. We later add a collisionless BH.
...
if rank == 0:
  ...
  blank_Typ = np.full(N_blank, -1) # Type of blank is set to -1 so that they are distinct from gas (Typ = 0) and collisionless (Typ = 1) particles!
  ...
  Typ = np.hstack(Typ, blank_Typ)
	...
	u[N] = 0.0
	h[N] = 0.0
	epsilon[N] = 0.001 # equivalent to 1 pc in code unit. Recall that unitLength is 1 kpc!
