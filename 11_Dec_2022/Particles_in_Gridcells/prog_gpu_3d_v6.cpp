...
int main()
{
    ...
    countParticlesInCells<<<gridSize, blockSize>>>(d_x, d_y, d_z, max_dist, GridSize, particleCounts, N);
    cudaDeviceSynchronize();  // ensure kernel completion before moving to the next
    
    // Added these two lines after `populateGrid` kernel
    ...
    populateGrid<<<gridSize, blockSize>>>(d_x, d_y, d_z, max_dist, GridSize, d_grid, d_tempParticleCounts, N);
    ...
    int i = 100;  // Note that this is cell number 100 and not particle with index 100 !!!!
