...
// Grid approach implemented to the acc_sph, divV_curl, and get_dU functions (26 Oct 2023).
...
__device__ int3 get_grid_cell_device(float x_p, float y_p, float z_p, float max_dist, int GridSize) {
  int cell_x = min(GridSize - 1, max(0, static_cast<int>(x_p * GridSize / max_dist)));
  int cell_y = min(GridSize - 1, max(0, static_cast<int>(y_p * GridSize / max_dist)));
  int cell_z = min(GridSize - 1, max(0, static_cast<int>(z_p * GridSize / max_dist)));
...
//===== populateGrid
__global__ void populateGrid2(float *xx, float *yy, float *zz, float *h, float max_dist, int GridSize, GridCell *grid, int N)
{
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < N)
  {
  
    float binSize = 1.5f * h[i];
    int GridSize = static_cast<int>(max_dist/binSize) + 1;
    
    int3 cell = get_grid_cell_device(xx[i], yy[i], zz[i], max_dist, GridSize);
...
//====================== acc_sph_grid ============================
//===========================================================
__global__ void acc_sph_grid(int *Typ, float *x, float *y, float *z, float *vx, float *vy, float *vz,
                            float *h, float *c, float *rho, float *divV, float *curlV,
                            float *mass, float *P, float *ax, float *ay, float *az,
                            float *xx, float *yy, float *zz, float max_dist, int GridSize, GridCell *grid,
                            int* neighboringParticles, const int maxNeighbors, float visc_alpha, int N)
{
  ...
  if ((i < N) && (Typ[i] == 0))
  {
    ...
    //---- Finding neighboringParticles for the particle of index i.
    int3 particle_cell = get_grid_cell_device(xx[i], yy[i], zz[i], max_dist, GridSize);
    ...
    for (int dxx = -1; dxx <= 1; ++dxx)
    {
      for (int dyy = -1; dyy <= 1; ++dyy)
      {
        ...
        {
          int neighbor_x = particle_cell.x + dxx;
          int neighbor_y = particle_cell.y + dyy;
          int neighbor_z = particle_cell.z + dzz;

          if (neighbor_x >= 0 && neighbor_x < GridSize && neighbor_y >= 0 && neighbor_y < GridSize && neighbor_z >= 0 && neighbor_z < GridSize)    
          {
    ...
    for (int jj = 0; jj < neighborsCount; jj++) 
    {
      int j = neighboringParticles[i * maxNeighbors + jj];
...
//================== div_curlVel_grid ======================
//=====================================================
__global__ void div_curlVel_grid(int *Typ, float *divV, float *curlV, float *x, float *y, float *z,
                            float *vx, float *vy, float *vz, float *rho, float *mass,
                            float *xx, float *yy, float *zz, float max_dist, int GridSize, GridCell *grid,
                            int* neighboringParticles, const int maxNeighbors,
                            float *h, int N)
{
  int i = threadIdx.x + blockIdx.x * blockDim.x;
...
//=================== get_dU Grid ====================
//===============================================
__global__ void get_dU_grid(int *Typ, float *x, float *y, float *z, float *vx, float *vy, float *vz,
                            float *h, float *c, float *rho, float *divV, float *curlV,
