...
const int MAX_PARTICLES_PER_CELL = 2000;  // This is a guess. Adjust based on your data.!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                            ...
                            int* neighboringParticles, const int maxNeighbors, float visc_alpha, int N)
{
  ...
  if ((i < N) && (Typ[i] == 0))
  {
    ...
    while (neighborsCount < 100)
    {
      for (int dx = -wGrid; dx <= wGrid; ++dx)
      {
        for (int dy = -wGrid; dy <= wGrid; ++dy)
        {
          for (int dz = -wGrid; dz <= wGrid; ++dz)
          {
      ...
      if (neighborsCount < 100) // extend to more neigboring cells and try again!!
      {
        neighborsCount = 0;
        wGrid++;
      }
