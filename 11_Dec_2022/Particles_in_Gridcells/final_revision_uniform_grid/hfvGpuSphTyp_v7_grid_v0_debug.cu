...
#include <tuple>
#include <algorithm>
#include "hfvCLibs_v7_grid_v0_debug2.h"
...
int main()
{
  ...
  float dt = 5e-6; //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! This is only the first time step !!
...
//--------------------------------------------
  ...
  for (int i = 0; i < N; i++)
  {
    ...
    vx[i] = 10.0f * x[i]; // vxvec[i];
    vy[i] = 10.0f * y[i]; // vyvec[i];
    vz[i] = 10.0f * z[i]; // vzvec[i];

    mass[i] = 100.0f; //massvec[i];
    eps[i] = epsvec[i];
  ...
  while (t < tEnd)
  {
    ...
    // Adjusting xx, yy, and zz
    minX = *min_element(xx, xx + N);
    minY = *min_element(yy, yy + N);
    minZ = *min_element(zz, zz + N);
    ...
    maxX = *max_element(xx, xx + N);
    maxY = *max_element(yy, yy + N);
    maxZ = *max_element(zz, zz + N);

    max_dist = max({maxX, maxY, maxZ});

    cout << "max_dist: " << max_dist << endl;
    
    min_h = findNonZeroPositiveMin(h, N);
    //min_h += 0.05f * min_h; // To make sure that the d_grid array has always more than enough space!
    binSize = 1.4f * min_h;
    ...
    //!!!! NOTE: At the end of each time step we do: "cudaFree(d_grid);". This is important!!!
    GridCell *d_grid;
    ...
    populateGrid2<<<gridSize, blockSize>>>(d_xx, d_yy, d_zz, d_h, max_dist, GridSize, d_grid, N);
    cudaDeviceSynchronize();
    ...
    */
    ...
    //if (counter > -1)
    {
      ...
      std::string filename = "./OutputsWG/G-" + to_string(t * 10) + ".bin";
    ...
    */
    ...
    checkCudaErrors(cudaFree(d_grid), "Free d_grid");
