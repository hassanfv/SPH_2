...
const int MAX_PARTICLES_PER_CELL = 500;
...
__device__ int3 get_grid_cell_device(float x_p, float y_p, float z_p, float max_dist, int GridSize) {
    ...
    cell_x = min(cell_x, GridSize - 1);
    cell_y = min(cell_y, GridSize - 1);
    cell_z = min(cell_z, GridSize - 1);
...
__global__ void countParticlesInCells(float *x, float *y, float *z, float max_dist, int GridSize, int *particleCounts, int N)
{
    ...
    if (i < N)
    {
        ...
        atomicAdd(&particleCounts[linearIndex], 1);
    }
}


//===== populateGrid
__global__ void populateGrid(float *x, float *y, float *z, float max_dist, int GridSize, GridCell *grid, int *particleCounts, int N)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < N)
    {
        ...
        int idx = atomicAdd(&particleCounts[linearIndex], -1) - 1;
        if (idx >= 0 && idx < MAX_PARTICLES_PER_CELL)
        {
            grid[linearIndex].particles[idx] = i;
            atomicAdd(&grid[linearIndex].count, 1);
        }
    }
}
