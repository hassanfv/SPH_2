...
vector<int> getNeighborsParticles(int i_p, float *x, float *y, float *z, float max_dist, int GridSize, int* cell_particles_offsets, int* cell_particles_values) 
{
...
int main() {
    ...
    vector<int> neighbors = getNeighborsParticles(i_p, x, y, z, max_dist, GridSize, cell_particles_offsets, cell_particles_values);
    cout << "Neighboring particles for the first particle: " << neighbors.size() << endl;




    ofstream outfile("data3d.bin", ios::binary);

    // Write N and GridSize at the beginning of the file
    ...
    // Write cell_particles_offsets and cell_particles_values arrays
    outfile.write(reinterpret_cast<const char*>(cell_particles_offsets), (GridSize * GridSize * GridSize + 1) * sizeof(int));
    outfile.write(reinterpret_cast<const char*>(cell_particles_values), N * sizeof(int));

    // Write the neighbors vector
    int neighbors_size = neighbors.size();
    outfile.write(reinterpret_cast<const char*>(&neighbors_size), sizeof(neighbors_size)); // Write the size of the neighbors vector
    outfile.write(reinterpret_cast<const char*>(neighbors.data()), neighbors_size * sizeof(int)); // Write the actual data of the neighbors vector
