...
#include "hfv_BHLibs_v1.h" // BH tree libs.
...
//--------------------------------------------
  ...
  cudaMalloc((void **)&d_b_buffer, sizeof(Body) * nBodies); // n = nBodies
  
  
  //--- preparing bodies (bodies are actually particles!) ---
  //random_device rd;  // Obtain a random number from hardware
  mt19937 eng(42); // Seed the generator
  uniform_real_distribution<> distr(-0.4, 0.4); // Define the range
  ...
  float accxT = b1.acceleration.x;
  float accyT = b1.acceleration.y;
  float acczT = b1.acceleration.z;
  ...
  cout << "Exited as planned AAA !!!" << endl;
  exit(0);
