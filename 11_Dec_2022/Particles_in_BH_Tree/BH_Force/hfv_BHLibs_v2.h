...
__device__ void ComputeForce(Node *node, Body *bodies, int nodeIndex, int bodyIndex, int nNodes, int nBodies, int leafLimit, float width)
{
  ...
  float q, q2, q3, q4, q5, q6, epsij, inv_r3;
  ...
  if (curNode.isLeaf)
  {
    if (curNode.centerMass.x != -1)
    {
      ...
      bodies[bodyIndex].acceleration.x += G * fk * rij.x * curNode.totalMass;
      bodies[bodyIndex].acceleration.y += G * fk * rij.y * curNode.totalMass;
      bodies[bodyIndex].acceleration.z += G * fk * rij.z * curNode.totalMass;
    }
    return;
  }
