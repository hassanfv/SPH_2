...
__global__ void maxCoord(float *d_x, float *d_y, float *d_z, float *d_max, int N)
{
...
__host__ __device__ int getCelliD(float x, float y, float z, float x_min, float y_min, float z_min, float Wcell, int nSplit)
{
    ...
    return depth * nSplit * nSplit + col * nSplit + row;
}


__device__ void CountBodies(float *d_x, float *d_y, float *d_z, float x_min, float y_min, float z_min, float Wcell, int nSplit, int *count, int Ncell, int N)
{
  int tx = threadIdx.x;
  if (tx < Ncell)
    count[tx] = 0;
...
__device__ void GroupBodies(float *d_x, float *d_y, float *d_z, int *d_groupedIndex, float x_min, float y_min, float z_min,
                          ...
                          float x_min, float y_min, float z_min, float W_cell, int nSplit, int Ncell, int N)
{
  ...
  CountBodies(d_x, d_y, d_z, x_min, y_min, z_min, W_cell, nSplit, count, Ncell, N);
  ComputeOffset(count, Ncell);
  GroupBodies(d_x, d_y, d_z, d_groupedIndex, x_min, y_min, z_min, W_cell, nSplit, count, Ncell, N);
...
int main()
{
  ...
  int cell_t = getCelliD(x[i], y[i], z[i], x_min, y_min, z_min, W_cell, nSplit);
  
  cout << "particle i is at cell = " << cell_t << endl;
