...
__global__ void ngbDB_v2(int *Typ, float *x, float *y, float *z, float *h, int *ngb, int MAX_ngb, int N)
{
  ...
  if ((i < N) && (Typ[i] == 0))
  {
    ...
    if (i == 55514)
      printf("i, k, counter h_new = %d, %d, %f, %f, %f, %f, %f\n",
              i, k, counter, x[i], y[i], z[i], h_new);
    ...
    while (k < 65)
    {
      ...
      while (j < N && k < MAX_ngb)
      {
        if (Typ[j] == 0)
        {
          ...
          /*
          if (i == 55514)
            printf("i, k, counter rr, h_new = %d, %d, %f, %f, %f, %f, %f, %f\n",
                    i, k, counter, rr, dx, dy, dz, h_new);
          */
                                ...
                                float Nngb_f, float *Nngb_previous, float *divV, int *ngb, int MAX_ngb, int *ngbDebug, float dt)
{
  ...
  if ((i < N) && (Typ[i] == 0))
  {
    ...
    while ((k < Ndown) || (k > Nup))
    {
      ...
      if (N_iter > 2000)
      {
        printf("Maximum N_iter REACHED in smoothing_h_ngb !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
        break;
      }
    }
  ...
  if ((i < N) && (Typ[i] == 0))
  {
    ...
    for (int jj = 0; jj < MAX_ngb; jj++)
    {
      ...
      if (j != -1) // Note that the Typ[j] == 0 condition was already checked during ngb array construction!
      {
        ...
        /*
        if (i == 48997)
            printf("j, rr, 2*hij, 2*h[i], 2*h[j] = %d, %f, %f, %f, %f\n", j, rr, 2*hij, 2*h[i], 2*h[j]);
        */
                        ...
                        int *ngb, int MAX_ngb, float visc_alpha, int N)
{
  ...
  if ((i < N) && (Typ[i] == 0))
  {
    ...
    if (i == 55514)
      printf("x[i], vx[i], h[i], c[i], rho[i], divV[i], curlV[i], mass[i], P[i], ax[i] = %f, %f, %f, %f, %f, %f, %f, %f, %f, %f\n\n\n",
              x[i], vx[i], h[i], c[i], rho[i], divV[i], curlV[i], mass[i], P[i], ax[i]);
    ...
    for (int jj = 0; jj < MAX_ngb; jj++)
    {
      ...
      if (j != -1) // Note that the Typ[j] == 0 condition was already checked during ngb array construction!
      {
        ...
        if (rr < 2.0f * hij)
        {
          ...
          /*
          if (i == 55514)
            printf("i, j, axt, mass[j], P[i], P[j], PIij, rho[i], rho[j], gWx = %d, %d, %f, %f, %f, %f, %f, %f, %f, %f\n",
                    i, j, axt, mass[j], P[i], P[j], PIij, rho[i], rho[j], gWx);
          */
        }
      }
    }
