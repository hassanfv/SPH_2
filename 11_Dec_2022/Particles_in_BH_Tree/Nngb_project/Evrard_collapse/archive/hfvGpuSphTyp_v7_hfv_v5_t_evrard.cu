...
#include <random>
#include <tuple>
#include "hfvCLibs_v7_hfv_v2_evrard.h" // I used my grid method.
#include "hfv_BHLibs_v2_evrard.h" // BH tree libs.
#include "ngb_v1_evrard.h"
...
//--------------------------------------------
  ...
  maxArr = new float;
  cudaMalloc((void **)&d_maxArr, sizeof(float));
  
  maxCoord<<<gridSize, blockSize, gridSize * sizeof(int)>>>(d_x, d_y, d_z, d_Typ, d_maxArr, N);
  cudaDeviceSynchronize();
  cudaMemcpy(maxArr, d_maxArr, sizeof(float), cudaMemcpyDeviceToHost);
  ...
  */
  ...
  cudaDeviceSynchronize();
  cudaMemcpy(maxArr, d_maxArr, sizeof(float), cudaMemcpyDeviceToHost);
  cout << "First maxArr[0] = " << maxArr[0] << endl;
  ...
  */
  ...
  blockSize3 = BLOCK_SIZE;
  gridSize3 = ceil((float)nBodies / blockSize3);
  ConstructQuadTreeKernel<<<1, BLOCK_SIZE>>>(d_node, d_b, d_b_buffer, 0, nNodes, nBodies, leafLimit);
  cudaDeviceSynchronize();
  ...
  cudaDeviceSynchronize();
  cudaMemcpy(maxArr, d_maxArr, sizeof(float), cudaMemcpyDeviceToHost);
  cout << "Second maxArr[0] = " << maxArr[0] << endl;
  cudaMemcpy(x, d_x, N * sizeof(float), cudaMemcpyDeviceToHost);
  for (int i = 0; i < 10; i++)
  {
    cout << "y[i] = " << y[i] << endl;
  ...
  */
  ...
  for (int i = 0; i < nBodies; i++)
  {
    ...
    accy[j] = b_tmp.acceleration.x;
    accz[j] = b_tmp.acceleration.x;
  }
