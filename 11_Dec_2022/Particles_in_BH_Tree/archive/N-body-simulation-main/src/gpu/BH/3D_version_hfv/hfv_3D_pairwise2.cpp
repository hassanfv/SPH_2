...
int main()
{
  ...
  //int i = 100; // Index of the body for which we want to calculate the acceleration
  for (int i = 0; i < nBodies; i+=100)
  {
    ...
    cout << endl;
    if (abs(b1.position.x) > 0.90 && abs(b1.position.y) > 0.45 && abs(b1.position.y) < 0.55 && abs(b1.position.z) < 0.02)
      exit(0);
  }
