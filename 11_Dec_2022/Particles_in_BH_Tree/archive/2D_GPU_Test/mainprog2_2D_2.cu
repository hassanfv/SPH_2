...
struct Parameters
{
  ...
  __host__ __device__ Parameters(int nbead)
  {
    ...
    max_depth = 1000000; // You may change it!!!
    min_points = 2; // You may change it!!!
...
__global__ void build_octree_kernel(Octree_node *nodes, Octree_node *all_nodes, int *num_nodes, int *points1, int *points2, FLOAT2 *dev_unc_pos, Parameters params)
{
  ...
  if(threadIdx.x == blockDim.x-1)
  {
    ...
    build_octree_kernel<<<4, blockDim.x, 8*sizeof(int)>>>(children, all_nodes, num_nodes, points1, points2, dev_unc_pos, Parameters(params, true));
  }
}
...
int main()
{
  ...
  int expected_number_of_nodes = 1000000;
