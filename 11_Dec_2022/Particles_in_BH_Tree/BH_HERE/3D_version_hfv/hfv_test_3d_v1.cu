...
__global__ void ComputeBoundingBoxKernel(Node *node, Body *bodies, int *mutex, int nBodies)
{
    ...
    __shared__ float topLeftX[BLOCK_SIZE];
    __shared__ float topLeftY[BLOCK_SIZE];
    __shared__ float botRightX[BLOCK_SIZE];
    __shared__ float botRightY[BLOCK_SIZE];
...
----------------------------------------------------------------------------------------
*/
__device__ int getQuadrant(Vector topLeft, Vector botRight, float x, float y)
{
...
__device__ void warpReduce(volatile float *totalMass, volatile float2 *centerMass, int tx)
{
...
__device__ void ComputeCenterMass(Node &curNode, Body *bodies, float *totalMass, float2 *centerMass, int start, int end)
{
    int tx = threadIdx.x;
...
__global__ void ConstructQuadTreeKernel(Node *node, Body *bodies, Body *buffer, int nodeIndex, int nNodes, int nBodies, int leafLimit)
{
    ...
    __shared__ float2 centerMass[BLOCK_SIZE];
    int tx = threadIdx.x;
