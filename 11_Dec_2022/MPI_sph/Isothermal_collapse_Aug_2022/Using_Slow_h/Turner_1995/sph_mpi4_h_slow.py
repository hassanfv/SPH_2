...
#----- P_polytrop_mpi
@njit
def P_polytrop_mpi(nbeg, nend, rho, rho_0, c_0, c_s):
	...
	for i in range(nbeg, nend):
		...
		if rhot <= rho_0:
			...
			P_res[i-nbeg] = rhot * ((c_0**2 - c_s**2)*(rhot/rho_0)**(-2./3.)+c_s**2)
...
#----- sound_speed_mpi
@njit
def sound_speed_mpi(nbeg, nend, rho, rho_0, c_0, c_s):
	...
	for i in range(nbeg, nend):
		...
		if rhot <= rho_0:
			...
			c[i-nbeg] = ((c_0**2 - c_s**2)*(rhot/rho_0)**(-2./3.)+c_s**2)**0.5
...
with open('IC_Kitsionas_12k.pkl', 'rb') as f:
	data = pickle.load(f)
...
#--------- p ----------
P = 0.
local_P = P_polytrop_mpi(nbeg, nend, rho, rho_0, c_0, c_s)
...
#--------- c ----------
c = 0.
local_c = sound_speed_mpi(nbeg, nend, rho, rho_0, c_0, c_s)
