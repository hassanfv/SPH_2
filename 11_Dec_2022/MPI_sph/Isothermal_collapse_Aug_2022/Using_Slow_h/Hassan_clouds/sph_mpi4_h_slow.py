...
@njit
def P_polytrop_mpi(nbeg, nend, rho, rho_crit, c_0, gamma):
	...
	for i in range(nbeg, nend):
		...
		P_res[i-nbeg] = rhot * c_0*c_0 * (1. + (rhot/rho_crit)**(gamma - 1.))
...
#----- sound_speed_mpi
@njit
def sound_speed_mpi(nbeg, nend, rho, rho_crit, c_0, gamma):
	...
	for i in range(nbeg, nend):
		...
		c[i-nbeg] = c_0 * (1. + (rhot/rho_crit)**(gamma - 1.))**0.5
...
with open('IC_Hassan_33k_UNIFORM.pkl', 'rb') as f:
	data = pickle.load(f)
...
c_0 = 32813.3 #* (Mcld/M_sun)**0.25 # cm/s
#c_s = 0.2 * 1e5 # cm/s
#rho_0 = 1e-23 # We over-ride rho_0 from above !
rho_crit = 5e-14 # g/cm^3
...
#--------- p ----------
P = 0.
local_P = P_polytrop_mpi(nbeg, nend, rho, rho_crit, c_0, gamma)
...
#--------- c ----------
c = 0.
local_c = sound_speed_mpi(nbeg, nend, rho, rho_crit, c_0, gamma)
...
while t < tEnd:
	...
	epsilon = h.copy()
	#epsilon = np.zeros_like(h) + 0.0005
