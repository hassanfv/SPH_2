...
#----- P_polytrop_mpi (EoS from Bhattal et al 1998)
@njit
def P_polytrop_mpi(nbeg, nend, rho, rho_0, rho_1, c_0, c_s):
	...
	for i in range(nbeg, nend):
		...
		if rhot > rho_0:
			P_res[i-nbeg] = rhot * (((c_0*c_0 - c_s*c_s)*(rhot/rho_0)**(-2./3.) + c_s*c_s)**2 +
						(((c_0*c_0 - c_s*c_s)*(rho_1/rho_0)**(-2./3.) + c_s*c_s)*(rhot/rho_1)**(gammax-1.0))**2)**0.5
...
def sound_speed_mpi(nbeg, nend, rho, rho_0, rho_1, c_0, c_s):
	...
	for i in range(nbeg, nend):
		...
		if rhot > rho_0:
			c[i-nbeg] = ((((c_0*c_0 - c_s*c_s)*(rhot/rho_0)**(-2./3.) + c_s*c_s)**2 +
						(((c_0*c_0 - c_s*c_s)*(rho_1/rho_0)**(-2./3.) + c_s*c_s)*(rhot/rho_1)**(gammax-1.0))**2)**0.5)**0.5
...
with open('IC_Bhattal_64k.pkl', 'rb') as f:
	data = pickle.load(f)

r = data['r']
v = data['v'] # For Kitsionas the division by unitVelocity must be done ! It is done below after defining the Unit Velocity !
#h = data['h']
m = data['m']
...
UnitRadius_in_pc = data['Rcld_in_pc']
Mcld_in_g = data['Mcld_in_g'] # in gram
Rcld_in_cm = data['Rcld_in_cm']

nGas = data['nGas']     # hydrodynamic + gravity
nICM = data['nICM']     # Only hydrodynamic acceleration.
nRigid = data['nRigid'] # Only hydrodynamic, however, their position is always fixed.
...
v = v / unitVelocity # This must be done Only for Kitsionas. For others it depends if you divide in IC creation or not ! Simple to check !

unitTime = (UnitRadius_in_cm**3/grav_const_in_cgs/UnitMass_in_g)**0.5
t_ff = unitTime # In Kistsionas paper, t_ff is not used so we do not need it so we set it to unitTime.
unitTime_in_Myr = unitTime / 3600. / 24. / 365.25 / 1.e6

print('Mcld/M_sun = ', Mcld_in_g/M_sun)
print('UnitRadius_in_cm = ', UnitRadius_in_cm)
print('UnitRadius_in_pc = ', UnitRadius_in_pc)
...
print('rho_0 (rho_cen) = ', rho_0)

#---- Constants -----------
eta = 0.1
gamma = gammax = 7./5. # 5.0/3.0# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! ATTENTION ATTENTION ATTENTION !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
mH = 1.6726e-24 # gram
...
c_0 = 0.2 * 1e5 * (Mcld_in_g/M_sun)**0.25 # cm/s
c_s = 0.2 * 1e5 # cm/s
rho_1 = 1e-14 # g/cm^3

#AAA = (2.*gamma*Mach**2 - (gamma - 1.)) * ((gamma - 1.) * Mach**2 + 2.)
#BBB = (gamma + 1.)**2 * Mach**2
#T_ps = AAA/BBB * T_cld

print('c_0 (effective thermal sound speed) = ', c_0)
...
#---- Setting the gravitational acceleration of ICM and Rigid particles to zero -----
acc_g[nGas, :] = 0.0
acc_g[nRigid, :] = 0.0
#----------------

abs_acc = (acc_g[:, 0]*acc_g[:, 0] + acc_g[:, 1]*acc_g[:, 1] + acc_g[:, 2]*acc_g[:, 2])**0.5
dt_f = np.min((h/(abs_acc+1e-20))**0.5)
...
#---- Setting the acc_sph of the Rigid particles to zero -----
acc_sph[nRigid] = 0.0
#----------------------
...
while t < tEnd:
	...
	if rank == 0:
		...
		dictx = {'pos': r, 'v': v, 'm': m, 'dt': dt, 'current_t': t, 'rho': rho, 'h': h,
			 ...
			 'Rcld_in_cm': Rcld_in_cm, 'Mcld_in_g': Mcld_in_g,
