...
def P_polytrop_mpi(nbeg, nend, rho, T_cld, T_ps, T_0):
	...
	for i in range(nbeg, nend):
		...
		if rhot > rho_cen:
			P_res[i-nbeg] = rhot * kBmH2 * T_0 * (2.06e-20 / rhot + 1. + (2.4e9)*rhot**(2./3.)) # Anathpindika thesis eq. 4.15.
...
def sound_speed_mpi(nbeg, nend, rho, T_cld, T_ps, T_0):
	...
	for i in range(nbeg, nend):
		...
		if rhot > rho_cen:
			c[i-nbeg] = (kBmH2 * T_0 * (2.06e-20 / rhot + 1. + (2.4e9)*rhot**(2./3.)))**0.5
...
with open('IC_Anathpindika_19k_RAND.pkl', 'rb') as f:
	data = pickle.load(f)
