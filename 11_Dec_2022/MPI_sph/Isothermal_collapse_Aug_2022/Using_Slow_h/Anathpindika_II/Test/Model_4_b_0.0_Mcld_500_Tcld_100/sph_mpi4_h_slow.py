...
@njit
def P_polytrop_mpi(nbeg, nend, rho, rho_crit, T_0, T_cld):
	...
	for i in range(nbeg, nend):
		...
		if rhot <= 3.0e-22:
			P_res[i-nbeg] = rhot * kB/mH2 * T_cld
		
		else:
			P_res[i-nbeg] = rhot * kB/mH2 * T_0 * (1.0 + (rhot/rho_crit)**(gamma - 1.0))
...
#----- sound_speed_mpi
@njit
def sound_speed_mpi(nbeg, nend, rho, rho_crit, T_0, T_cld):
	...
	for i in range(nbeg, nend):
		...
		if rhot <= 3.0e-22:
			c[i-nbeg] = (kB/mH2 * T_cld)**0.5
			
		else:
			c[i-nbeg] = (kB/mH2 * T_0 * (1.0 + (rhot/rho_crit)**(gamma - 1.0)))**0.5
...
with open('IC_Anathpindika_5k_RAND.pkl', 'rb') as f:
	data = pickle.load(f)
...
muu = data['mu']
Mach = data['Mach']
#c_0 = data['c_0'] # isothermal sound speed

UnitRadius_in_pc = data['Rcld_in_pc']
Mcld = data['Mcld'] # in gram
...
G = grav_const_in_cgs = 6.67259e-8 #  cm3 g-1 s-2
...
v = v / unitVelocity # This must be done Only for Kitsionas + Anathpindika. For others it depends if you divide in IC creation or not ! Simple to check !
...
#T_ps = T_0 # AAA/BBB * T_cld

#print('T_ps = ', T_ps)

#c_0 = (kB * T_0 / mH2)**0.5 # Isothermal sound speed

#print('c_0 (isothermal thermal sound speed) = ', c_0)
...
#--------- p ----------
P = 0.
local_P = P_polytrop_mpi(nbeg, nend, rho, rho_crit, T_0, T_cld)
...
#--------- c ----------
c = 0.
local_c = sound_speed_mpi(nbeg, nend, rho, rho_crit, T_0, T_cld)
