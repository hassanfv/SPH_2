...
import pickle
import os
from libsx2_1 import *
from mpi4py import MPI
from shear_test3 import *
...
#---- READING CONSTANTS ------------
with open('IC_Anathpin_10k_RAND.pkl', 'rb') as f:
	data = pickle.load(f)
...
filz = np.sort(os.listdir('./Outputs'))
last_file_name = filz[-1]

ii = int(last_file_name.split('.')[0])

with open('./Outputs/' + last_file_name, 'rb') as f:
	data = pickle.load(f)
...
epsilon = np.zeros_like(h) + 0.0001
...
while t < tEnd:
	...
	local_h = smoothing_fast_new_mpi(nbeg, nend, r, h_previous)
	#local_h = smoothing_length_mpi(nbeg, nend, r)
