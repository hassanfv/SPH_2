...
# The difference with shear_test2.py is that here we optmize "getAcc_sph_shear_mpi" to only uese within this range (i.e rad_h = 3.0 * h[i]) in the inner loop.
# The difference with shear_test.py is that here we replaced "vxij = v[i, 0] - v[j, 0]" with "vxij = -vxji". By this we could improve the speed a bit !
...
def div_curlVel(pos, v, rho, m, h):
	...
	for i in range(N):
		...
		for j in range(N):
			...
			vxij = -vxji #v[i, 0] - v[j, 0]
			vyij = -vyji #v[i, 1] - v[j, 1]
			vzij = -vzji #v[i, 2] - v[j, 2]
...
def getAcc_sph_shear_mpi(nbeg, nend, pos, v, rho, P, c, h, m, divV, curlV, alpha): # shear viscosity-corrected.
	...
	for i in range(nbeg, nend):
		...
		for j in range(MM):
			...
			#----- gradW section -----
			dx = pos[i, 0] - posNew[j, 0]
			dy = pos[i, 1] - posNew[j, 1]
			dz = pos[i, 2] - posNew[j, 2]
			rr = (dx**2 + dy**2 + dz**2)**0.5
			...
			wij = vij_rij / (rr+1e-5)
			vij_sig = c[i] + cNew[j] - 3.0 * wij
			rhoij = 0.5 * (rho[i] + rhoNew[j])
			...
			fj = divVNew[j]/(divVNew[j] + curlVNew[j] + 0.0001*cNew[j]/hNew[j])
			fij = 0.5 * (fi + fj)
			PIij = fij * PIij
			#-------------------------
			
			axt -= mNew[j] * (P[i]/rho[i]**2 + PNew[j]/rhoNew[j]**2 + PIij) * gWx
			ayt -= mNew[j] * (P[i]/rho[i]**2 + PNew[j]/rhoNew[j]**2 + PIij) * gWy
			azt -= mNew[j] * (P[i]/rho[i]**2 + PNew[j]/rhoNew[j]**2 + PIij) * gWz
...
@njit
def getAcc_sph_shear_mpiZZZZZZZZ(nbeg, nend, pos, v, rho, P, c, h, m, divV, curlV, alpha, beta, eta): # shear viscosity-corrected.
