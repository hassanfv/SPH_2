...
#with open('./Outputs_Me_Gadvisc_19k_Mach_3/00750.pkl', 'rb') as f:
#with open('./Outputs_Me_19k_Mach_3/00001.pkl', 'rb') as f:
with open('00230.pkl', 'rb') as f:
	data = pickle.load(f)
...
vx = np.round(v[:, 0], 5) #/ unitVelocity
vy = np.round(v[:, 1], 5) #/ unitVelocity
vz = np.round(v[:, 2], 5) #/ unitVelocity
	
dictx = {'x': x, 'y': y, 'z': z, 'vx': vx, 'vy': vy, 'vz': vz, 'm': m, 'h': h, 'eps': eps}

df = pd.DataFrame(dictx)

df.to_csv('GPU_IC_Test.csv', index = False)
