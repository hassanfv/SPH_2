...
for j in range(350, len(filz), 10):
	...
	G = 1.0
	gg = getAcc_g_smth(r, mass, G, epsilon)
	
	acc = (gg[:, 0]*gg[:, 0] + gg[:, 1]*gg[:, 1] + gg[:, 2]*gg[:, 2])**0.5
	...
	ax.scatter(xx, acc, s = 0.1, color = 'k')
	#ax.scatter(xx, PP, s = 0.1, color = 'k')
	#ax.scatter(xx, cc, s = 0.1, color = 'b')
	
	if len(nnx) > 0:
		ax.scatter(xx[nnx], acc[nnx], s = 5, color = 'red')
