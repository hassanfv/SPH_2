...
def P_polytrop_mpi(nbeg, nend, rho, rho_0, rho_1, c_0, c_s):
	...
	for i in range(nbeg, nend):
		...
		if rhot > rho_0:
			P_res[i-nbeg] = rhot * ((c_0*c_0 - c_s*c_s)*(rhot/rho_0)**(-2./3.) + c_s*c_s) * (1. + (rhot/rho_1)**(4./3.))**0.5
...
def sound_speed_mpi(nbeg, nend, rho, rho_0, rho_1, c_0, c_s):
	...
	for i in range(nbeg, nend):
		...
		if rhot > rho_0:
			c[i-nbeg] = (((c_0*c_0 - c_s*c_s)*(rhot/rho_0)**(-2./3.) + c_s*c_s) * (1. + (rhot/rho_1)**(4./3.))**0.5)**0.5
...
with open('IC_Kitsionas_12k_RAND.pkl', 'rb') as f:
	data = pickle.load(f)
