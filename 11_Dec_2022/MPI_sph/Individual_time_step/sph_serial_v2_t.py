...
while t < tEnd:
	...
	#--------- h ----------
	h[nG] = h_smooth_fast_itime(r, h, nG) # !!!!!!!
	#----------------------

	#-------- rho ---------
	rho[G] = getDensity_itime(r, m, h, nG) # !!!!!!!
	#----------------------

	#------- acc_g --------
	acc_g[nG] = getAcc_g_smth(r, m, G, epsilon, nG) # !!!!!!!
	#----------------------
	...
	#--- divV & curlV -----
	divV[nG], curlV[nG] = div_curlVel(r, v, rho, m, h, nG) # !!!!!!!
	#----------------------

	#------ acc_sph -------
	acc_sph = getAcc_sph_shear(r, v, rho, P, c, h, m, divV, curlV, alpha, nG) # !!!!!!!
	#----------------------
	...
	#--------- ut ---------
	ut[nG] = get_dU_shear(r, v, rho, P, c, h, m, divV, curlV, alpha, [nG]) # !!!!!!!
	#----------------------
	...
	nG0 = list(np.where((dtz < dt_i[0]))[0])
	nG2 = list(np.where((dtz >= dt_i[0]) & (dtz <  dt_i[1]))[0])
	nG4 = list(np.where((dtz >= dt_i[1]) & (dtz <  dt_i[2]))[0])
	nG8 = list(np.where((dtz >= dt_i[2]) & (dtz <  dt_i[3]))[0])
	nG16= list(np.where((dtz >= dt_i[3]) & (dtz <= dt_i[4]))[0])
