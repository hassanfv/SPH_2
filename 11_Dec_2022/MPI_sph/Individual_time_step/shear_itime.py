...
#===== divVel & curlVel
@njit
def div_curlVel_itime(pos, v, rho, m, h, nG):
	...
	for i in range(M):
		...
		for j in range(N):
			...
			#----- gradW section -----
			dx = pos[nG[i], 0] - pos[j, 0]
			dy = pos[nG[i], 1] - pos[j, 1]
			dz = pos[nG[i], 2] - pos[j, 2]
			rr = (dx**2 + dy**2 + dz**2)**0.5
			...
			curlVx += m[j]/rho[nG[i]] * (vyij * gWz - vzij * gWy)
			curlVy += m[j]/rho[nG[i]] * (vzij * gWx - vxij * gWz)
			curlVz += m[j]/rho[nG[i]] * (vxij * gWy - vyij * gWx)
...
#===== getAcc_sph_shear (Non-parallel) (with Gadget 2.0 and 4.0 PIij)
@njit
def getAcc_sph_shear_itime(pos, v, rho, P, c, h, m, divV, curlV, alpha, nG): # shear viscosity-corrected
	...
	for i in range(M):
		...
		for j in range(N):
			...
			wij = vij_rij / (rr+1e-5)
			vij_sig = c[nG[i]] + c[j] - 3.0 * wij
			rhoij = 0.5 * (rho[nG[i]] + rho[j])
			...
			#---- shear-visc. correction -----
			fi = divV[nG[i]]/(divV[nG[i]] + curlV[nG[i]] + 0.0001*c[nG[i]]/h[nG[i]])
			fj = divV[j]/(divV[j] + curlV[j] + 0.0001*c[j]/h[j])
			fij = 0.5 * (fi + fj)
			PIij = fij * PIij
			...
			axt -= m[j] * (P[nG[i]]/rho[nG[i]]**2 + P[j]/rho[j]**2 + PIij) * gWx
			ayt -= m[j] * (P[nG[i]]/rho[nG[i]]**2 + P[j]/rho[j]**2 + PIij) * gWy
			azt -= m[j] * (P[nG[i]]/rho[nG[i]]**2 + P[j]/rho[j]**2 + PIij) * gWz
...
#===== get_dU_shear (Non-parallel) (with Gadget 2.0 and 4.0 PIij)
@njit
def get_dU_shear_itime(pos, v, rho, P, c, h, m, divV, curlV, alpha, nG): # shear-corrected
	...
	for i in range(M):
		...
		for j in range(N):
			...
			du_t += m[j] * (P[nG[i]]/rho[nG[i]]**2 + PIij/2.) * vij_gWij
