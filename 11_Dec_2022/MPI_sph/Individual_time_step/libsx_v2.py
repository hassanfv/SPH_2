...
# The difference with libsx,py is that here we also have functions for the individual time step algorithms.
...
#===== getDensity_itime
@njit
def getDensity_itime(pos, m, h, nG):  # You may want to change your Kernel !!
...
#===== getAcc_g_smth_itime
@njit
def getAcc_g_smth_itime(pos, mass, G, epsilon, nG):
	...
	for i in range(M):
		...
		for j in range(N):
			...
			dx = pos[j, 0] - pos[nG[i], 0]
			dy = pos[j, 1] - pos[nG[i], 1]
			dz = pos[j, 2] - pos[nG[i], 2]
			
			rr = (dx*dx + dy*dy + dz*dz)**0.5

			inv_r3 = 1.0 / (rr**3+1e-7)

			epsilonij = 0.5 * (epsilon[nG[i]] + epsilon[j])
			q = rr / epsilonij
...
#===== h_smooth_fast 
@njit
def h_smooth_fast_itime(pos, h, nG):
